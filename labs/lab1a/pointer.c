...
int int_size() {
  ...
  return (long)int_ptr2 - (lon)int_ptr1;
}
...
int within_array(int* int_array, int size, int* ptr) {
  long fromStart;
  long toEnd;
  fromStart = (long)ptr - (long)int_array;
  toEnd = (long)(int_array + size) - 1 - (long)ptr;

  //both should be positive, MSB should be 0
  return (fromStart >> 63) + (toEnd >> 63) == 0;
}
