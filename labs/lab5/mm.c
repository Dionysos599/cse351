...
void* mm_malloc(size_t size) {
  ...
  // Search the free list for a block of at least req_size
  ...
  // Check if the found block can be split
  block_size = SIZE(ptr_free_block->size_and_tags);
  if (block_size - req_size >= MIN_BLOCK_SIZE) { // Perform splitting
    ...
    split_block->size_and_tags &= ~TAG_USED;
    
    // Set footer of the split block
    *((size_t*) UNSCALED_POINTER_ADD(split_block, block_size - WORD_SIZE))= 
      split_block->size_and_tags;
    insert_free_block(split_block); // Insert the split block into the free list
    block_size = req_size;
  } else { // No splitting
    *((size_t*) UNSCALED_POINTER_ADD(ptr_free_block, SIZE(ptr_free_block->size_and_tags))) |= 
			   TAG_PRECEDING_USED;
  }

  remove_free_block(ptr_free_block); // Remove the block from the free list
  ...
  ptr_free_block->size_and_tags = preceding_block_use_tag 
    ? block_size | (TAG_PRECEDING_USED + TAG_USED)
    : block_size | TAG_USED;

  // Return a pointer to the payload of the allocated block
  return ptr_free_block == NULL ? NULL : (void*) UNSCALED_POINTER_ADD(ptr_free_block, WORD_SIZE);
}
...
void mm_free(void* ptr) {
  ...
  // Calculate block address and following block address from given pointer
  ...
  // Clear the used bit in the header to mark block as free
  block_to_free->size_and_tags &= ~TAG_USED;

  // Set the footer to match the header
  *((size_t*) UNSCALED_POINTER_ADD(block_to_free, payload_size - WORD_SIZE)) = block_to_free->size_and_tags;
  
  // Clear the preceding-used tag in the following block's header
  following_block->size_and_tags &= ~TAG_PRECEDING_USED;

  // Insert the now free block into the free list and coalesce it
